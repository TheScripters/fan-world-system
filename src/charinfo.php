...
$count = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS count FROM ".$table."charinfo"));//figures out fow many rows there are in the table "charinfo"
...
if ($_GET['mode'] == ""){//if the page has not been visited yet
	...
	for ($i=1; $i <= $barcount; $i++){
  	$charinfo = mysql_fetch_assoc(mysql_query("select * from ".$table."charinfo where id = $i"));//gets all the data from the particular row in "charinfo"
  	//generates the modify button and the last modified by info
	$user = mysql_fetch_array(mysql_query("SELECT user_id FROM ".$table."charinfo WHERE id = $i"));
	$user_name = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = ".$user['0'].""));
  	print "
		<tr>
			<td>
...
if ($_SESSION['logged_in']){
  	for ($x=1; $x <= $barcount; $x++){
    	if ($_GET['mode'] == $x){
    	  	$charinfo = mysql_fetch_assoc(mysql_query("select * from ".$table."charinfo where id = $x"));
...
if ($_SESSION['logged_in'] && $_GET['mode'] == "modify"){
    ...
    $charinfo = mysql_fetch_assoc(mysql_query("select name from ".$table."charinfo where id = ".$_REQUEST['who'].""));
    ...
    $sql = mysql_query("UPDATE ".$table."charinfo SET description = '".clean_it($_REQUEST['description_modify'])."' WHERE id = '".$_REQUEST['who']."'");
    $sql = mysql_query("UPDATE ".$table."charinfo SET user_id = '".$_SESSION['userID']."' WHERE id = '".$_REQUEST['who']."'");
