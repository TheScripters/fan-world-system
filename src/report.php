...
if (!$_GET['mode']){
  ...
  $link = mysql_fetch_array(mysql_query("SELECT LinkTitle FROM ".$table."links WHERE LinkID = '".$_GET['link']."'"));
?><html>
<head>
      ...
      if (valid_email($email))
        {
          if ($visual == $_SESSION['visual_confirmation'])
            {
              $link = mysql_fetch_array(mysql_query("SELECT LinkTitle FROM ".$table."links WHERE LinkID = '$linkid'"));
              mail("staff@kpfanworld.com","Broken Link Reported on ".$_SESSION['siteTitle']."",strip_gpc_slashes("Dear Staff,\n\n$name ($email) has the following broken link:\n\n".$link['LinkTitle']."\n\n--------\nThank you.\n\nIP: ".$_SERVER['REMOTE_ADDR'].""),$headers);
              mail($email,"Link Reported as Broken on ".$_SESSION['siteTitle']."",strip_gpc_slashes("Dear $name,\n\nThe following link has been reported to the staff of ".$_SESSION['siteTitle'].":\n\n".$link['LinkTitle']."\n-------\nThank you.\n\nIP: ".$_SERVER['REMOTE_ADDR'].""),$headers);
