...
if ($_SESSION['logged_in'] == "True")
  {
    $count = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS bans FROM ".$table."user_bans WHERE UserID = '".$_SESSION['userID']."'"));
    if ($count['bans'] == "1")
      {
        unset($_SESSION);
        session_destroy();
...
$NewDate = mysql_query("SELECT Effective_Until AS Effect,UserId FROM ".$table."user_bans");
while($date = mysql_fetch_array($NewDate))
 {
   if ($today >= $date['Effect'] && $date['Effect'] != "0")
     {
       $unban = mysql_query("DELETE FROM ".$table."user_bans WHERE UserID = '".$date['UserId']."'");
       $email = mysql_fetch_array(mysql_query("SELECT UserName,UserEmail FROM ".$table."users WHERE UserId = '".$date['UserId']."'"));
