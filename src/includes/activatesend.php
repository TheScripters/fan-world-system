...
/*******activatesend.php*********/
$send = mysql_fetch_array(mysql_query("SELECT Config_Value AS value FROM ".$table."config WHERE Config_ID = 'ActivateSend'"));
...
<br><br><center><a href=\"http://www.kpfanworld.com\"><img src=\"images/kimweb.png\" border=\"0\"></a></center>";
        mail($mail['UserEmail'],"Activation Reminder For ".$_SESSION['siteTitle']."",$header."<br><h1>Activation Reminder</h1><br><table align=\"center\" width=\"65%\">\n<tr>\n<td>Do not be alarmed by this message, ".$mail['UserName'].". This email is simply to remind you that you have not yet activated your account on ".$_SESSION['siteTitle'].". You can use the link below to do so:<br><br><a href=\"http://www.kpfanworld.com/activate.php?user=".$mail['UserId']."&code=".$mail['Act_Key']."\">http://www.kpfanworld.com/activate.php?user=".$mail['UserId']."&code=".$mail['Act_Key']."</a><br><br>If you are having issues activating your account try copying and pasting the activate URL into your browser's address bar or please contact <a href=\"mailto:staff@kpfanworld.com?subject=User Activation Issues\">staff@kpfanworld.com</a> for help<br><br>Thanks,<br>".$_SESSION['siteTitle']." Staff<br><a href=\"mailto:staff@kpfanworld.com\">staff@kpfanworld.com</a></td></tr></table><br><br><hr><h6>&copy; 2006 ".$_SESSION['siteTitle']."<br>\"Kim Possible\" &copy 2002-2006 Walt Disney</h6></body></html>",$headers_html);
      }
    $nextmail = mktime(18,30,0,$date[0],$date[1],$date[2]) + $cfgvalue[0];
    $update = mysql_query("UPDATE ".$table."config SET Config_Value = '".$cfgvalue[0].",".$today.",".$nextmail."' WHERE Config_ID = 'ActivateSend'");
  }
?>
