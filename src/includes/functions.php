...
(!isset($db)) ? include("includes/db.php") : include("includes/db".$db.".php");

$sitesess = array('SessionId'=>'kpfw');

// Begin session variables
ini_set("session.name",$sitesess['SessionId']);
session_start();

$_SESSION['siteEmail'] = "staff@kpfanworld.com";
$_SESSION['SiteURL'] = "fwsys.thescripters.com";
$_SESSION['siteTitle'] = "Fan World System Beta";

// Domain checking
if ($_SERVER['HTTP_HOST'] != $_SESSION['SiteURL'])
  {
    $uri = preg_replace("/~([a-z]+)/","",$_SERVER['REQUEST_URI']);
    header("HTTP/1.1 301 Moved Permanently");
    header("Location: http://".$_SESSION['SiteURL']."/".$uri);
  } elseif ($_SERVER['HTTP_HOST'] == $_SESSION['SiteURL'] && substr($_SERVER['REQUEST_URI'],0,10) == "/~([a-z]+)") {
    $uri = preg_replace("/~([a-z]+)/","",$_SERVER['REQUEST_URI']);
    header("HTTP/1.1 301 Moved Permanently");
    header("Location: http://".$_SESSION['SiteURL']."/".$uri);
  }
...
$IP_Ban = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS banned FROM ".$table."ip_bans WHERE Ban_IP = '".$_SERVER['REMOTE_ADDR']."'"));
if ($IP_Ban['banned'] == "1")
  {
    header("HTTP/1.1 403 Forbidden");
    echo "<html>\n<head>\n<title>".$_SESSION['siteTitle']." .:::. Error</title>\n<base href=\"http://www.kpfanworld.com/\">\n";
    echo "<link rel=\"stylesheet\" type=\"text/css\" href=\"includes/kpfw.css\">\n";
    echo "</head>\n\n<body><br><center><img src=\"images/kpfwlogo.jpg\"><br><br><font color=\"#FFFF00\" size=\"4\"><b>Your current IP (".$_SERVER['REMOTE_ADDR'].") has been banned.<br>If you feel this in error, please contact the management at <a href=\"mailto:".$_SESSION['siteEmail']."\">".$_SESSION['siteEmail']."</a>.<br><br>Thank You.</center>\n</body>\n</html>";
    exit;
  }
...
// Email headers
$_SESSION['headers'] = "Reply-to: ".$_SESSION['siteEmail']."\nFrom: ".$_SESSION['siteEmail']."\nReturn-Path: ".$_SESSION['siteEmail']."\nMessage-ID: <" . md5(time()) . "@".$_SESSION['SiteURL'].">\nMIME-Version: 1.0\nContent-type: text/plain; charset=iso-8859-1\nContent-transfer-encoding: 8bit\nDate: " . date('r', time()) . "\nX-Priority: 3\nX-MSMail-Priority: Normal\nX-Mailer: PHP\nX-MimeOLE: Produced By !!".$_SESSION['siteTitle']."!!\n";

function mail_user($uid,$active)
  {
    $userinfo = mysql_fetch_array(mysql_query("SELECT UserName,UserEmail FROM ".$table."users WHERE UserId = '".$uid."'"));
    $Active = ($active == "1") ? "activated" : "deactivated";
    mail($userinfo['UserEmail'],"Account $Active on !!".$_SESSION['siteTitle']."!!","Hello ".$userinfo['UserName'].",\n\nThis email is to inform you that your account has been $Active on !!".$_SESSION['siteTitle']."!!.\n\nIf this is not right, you may email us at ".$_SESSION['siteEmail']."\n\nThank you,\n!!".$_SESSION['siteTitle']."!! Management","Reply-to: ".$_SESSION['siteEmail']."\nFrom: ".$_SESSION['siteEmail']."\nReturn-Path: ".$_SESSION['siteEmail']."\nMessage-ID: <" . md5(time()) . "@kpfanworld.com>\nMIME-Version: 1.0\nContent-type: text/plain; charset=iso-8859-1\nContent-transfer-encoding: 8bit\nDate: " . date('r', time()) . "\nX-Priority: 3\nX-MSMail-Priority: Normal\nX-Mailer: PHP\nX-MimeOLE: Produced By !!".$_SESSION['siteTitle']."!!\n");
  }
  
function mail_admin($section,$old,$new)
  {
    $adminsql = mysql_query("SELECT UserName,UserEmail FROM ".$table."users WHERE UserLevel = '3' AND UserEmail != 'webmaster@kpfanworld.com'");
        if ($old != ""){
          mail("staff@kpfanworld.com",$section." Changed on !!".$_SESSION['siteTitle']."!!",stripslashes("Dear Staff Member,\n\nUser ".$_SESSION['UserName']." has updated information in the ".$section." section of !!".$_SESSION['siteTitle']."!!.\n\nOriginal entry:\n\n".$old."\n\n---\n\nNew Entry:\n\n".$new."\n\n--\nThank you,\n!!".$_SESSION['siteTitle']."!! Staff\nstaff@kpfanworld.com"),"Reply-to: staff@kpfanworld.com\nFrom: webmaster@kpfanworld.com\nReturn-Path: staff@kpfanworld.com\nMessage-ID: <" . md5(time()) . "@kpfanworld.com>\nMIME-Version: 1.0\nContent-type: text/plain; charset=iso-8859-1\nContent-transfer-encoding: 8bit\nDate: " . date('r', time()) . "\nX-Priority: 3\nX-MSMail-Priority: Normal\nX-Mailer: PHP\nX-MimeOLE: Produced By !!".$_SESSION['siteTitle']."!!\n");
        }
        else{
          mail("staff@kpfanworld.com",$section." Added to !!".$_SESSION['siteTitle']."!!",stripslashes("Dear Satff Member,\n\nUser ".$_SESSION['UserName']." has added information in the ".$section." section of !!".$_SESSION['siteTitle']."!!.\n\nNew Entry:\n\n".$new."\n\n--\nThank you,\n!!".$_SESSION['siteTitle']."!! Staff\nstaff@kpfanworld.com"),"Reply-to: staff@kpfanworld.com\nFrom: webmaster@kpfanworld.com\nReturn-Path: staff@kpfanworld.com\nMessage-ID: <" . md5(time()) . "@kpfanworld.com>\nMIME-Version: 1.0\nContent-type: text/plain; charset=iso-8859-1\nContent-transfer-encoding: 8bit\nDate: " . date('r', time()) . "\nX-Priority: 3\nX-MSMail-Priority: Normal\nX-Mailer: PHP\nX-MimeOLE: Produced By !!".$_SESSION['siteTitle']."!!\n");
        }
  }
...
function meta_tags()
  {
     $keywords_sql = mysql_query("SELECT Config_Value FROM ".$table."config WHERE Config_ID = 'Keywords'");
     $keywords = mysql_fetch_array($keywords_sql);
     echo "<meta name=\"keywords\" content=\"".strip_gpc_slashes($keywords['Config_Value'])."\">\n";
     $descrip_sql = mysql_query("SELECT Config_Value FROM ".$table."config WHERE Config_ID = 'Description'");
...
function censor($text)
  {
    $censor = mysql_fetch_array(mysql_query("SELECT Config_Value AS words FROM ".$table."config WHERE Config_ID = 'word_censor'"));
...
function privmsg($uid)
  {
    $PMCountNew = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS PMNew FROM ".$table."privmsg WHERE To_User = '$uid' AND New = '1'"));
    $PMCountTotal = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS PMTotal FROM ".$table."privmsg WHERE To_User = '$uid'"));
