...
if ($_GET['mode'] == "resize"){
	...
	if (substr($pic, strlen($pic)-4, 4)==".jpg"){
		...
		if (imagejpeg($image_p, $filename, 100))
		{
			...
			$sql = mysql_query("UPDATE ".$table."avhosting SET File_Size = '$new_file_size' WHERE AvFilename = '$pic'");
			header("Location: avhosting");
		}else{
	...
	{
	  	$sql = mysql_query("UPDATE ".$table."avhosting SET AvFilename = '$new_name' WHERE AvFilename = '$orig'");
		header("Location: avhosting");
	}else{
	  	incheader("Avatar Hosting");
	...
	{
		$file_size = mysql_fetch_array(mysql_query("SELECT * FROM ".$table."avhosting WHERE AvFilename = '$orig' and UserId = '".$_SESSION['userID']."'"));
		//adds the name of the file in the data base
		$sql = mysql_query("INSERT INTO ".$table."avhosting VALUES ('$new_name', '".$_SESSION['userID']."', '".$file_size['File_Size']."')");
		header("Location: avhosting");
	}else{
	  	incheader("Avatar Hosting");
