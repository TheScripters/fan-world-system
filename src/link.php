...
if (isset($_GET['id']))
  {
    $LinkID = mysql_fetch_array(mysql_query("SELECT LinkURL,LinkTitle,Hits FROM ".$table."links WHERE LinkID = '".$_GET['id']."'"));
    $OldHitValue = $LinkID['Hits'];
    $NewHitValue = $OldHitValue + 1;
    $Hits_Update = mysql_query("UPDATE ".$table."links SET Hits = '$NewHitValue' WHERE LinkID = ".$_GET['id']."");
