...
if (!empty($_REQUEST['newpassword']))
    ...
    if ($_REQUEST['newpassword'] == $_REQUEST['vrfypassword'])
      {
        ...
        $sql = mysql_query("UPDATE ".$table."users SET UserEmail = '".$_REQUEST['uemail']."', Time_Zone = '".$_REQUEST['timezone']."', Name = '".$_REQUEST['name']."', Profile = '".$profile."', Password = '".$newpass."' WHERE UserId = '".$_REQUEST['user']."'");
  ...
  {
    $profile = addslashes($_REQUEST['profile']);
    $sql = mysql_query("UPDATE ".$table."users SET UserEmail = '".$_REQUEST['uemail']."', Time_Zone = '".$_REQUEST['timezone']."', Name = '".$_REQUEST['name']."', Profile = '".$profile."' WHERE UserId = '".$_REQUEST['user']."'");
