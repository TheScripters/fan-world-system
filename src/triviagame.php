...
if (!$_GET['mode']){
	...
	<table border=\"0\" align=\"center\">
  	...
  	if ($_REQUEST['diff']!=""){
		$question_sql_query = (mysql_query("SELECT * FROM ".$table."trivia where Diff_Level = ".$_REQUEST['diff']." ORDER BY RAND() LIMIT 10"));
		$counter=0;
  ...
  $number_right=0;
  	for($x=1;$x<11;$x++){
  	  	...
  	  	if ($_REQUEST[$answer]!=""){
	  	  	...
	  	  	$question_sql_query = mysql_fetch_array(mysql_query("SELECT Poss_Ans_Full, Correct_Ans, Ques_Text FROM ".$table."trivia where Ques_ID = $info[1]"));
