...
if ($_SESSION['logged_in'] && !$_GET['mode'] && $_GET['id'])
  {
    ...
    $event = mysql_fetch_array(mysql_query("SELECT Month_Day,Year,News_Text FROM ".$table."today WHERE News_ID = '".$_GET['id']."'"));
 ...
 elseif ($_SESSION['logged_in'] && $_GET['mode'] && !$_GET['id'])
  {
    ...
    $sql = mysql_query("UPDATE ".$table."today SET Month_Day = '".$date[0]." ".$date[1]."', Year = '".$date[2]."', News_Text = '".$_REQUEST['event']."', Date = '".$today."', UserID = '".$_SESSION['userID']."' WHERE News_ID = '".$_REQUEST['News_ID']."'");
