...
if (!$_GET['mode']){
  $times = mysql_fetch_array(mysql_query("SELECT Config_Value AS times FROM ".$table."config WHERE Config_ID = 'countdown_time'"));
  $text = mysql_fetch_array(mysql_query("SELECT Config_Value AS text FROM ".$table."config WHERE Config_ID = 'countdown_text'"));
  $count_act = mysql_fetch_array(mysql_query("SELECT Config_Value as active FROM ".$table."config WHERE Config_ID = 'countdown_active'"));
  ...
  ?>      <center><font size="+2"><b>Server timezone: <?=date("T O e")?></b></font><br>Set accordingly!<br>
      ...
      <b>Date:</b> <input type="text" size="3" maxlength="2" name="mo_p" value="<?=$time_pst[3]?>"> <input type="text" size="3" maxlength="2" name="day_p" value="<?=$time_pst[4]?>">, <input type="text" size="3" maxlength="2" name="yr_p" value="<?=$time_pst[5]?>"><br><br>
      ...
      <b>Date:</b> <input type="text" size="3" maxlength="2" name="mo_e" value="<?=$time_est[3]?>"> <input type="text" size="3" maxlength="2" name="day_e" value="<?=$time_est[4]?>">, <input type="text" size="3" maxlength="2" name="yr_e" value="<?=$time_est[5]?>"><br><br>
      <b>Countdown Visible:</b> <input type="checkbox" name="active" value="True"<?=$active?>><br>
      <b>Length of event in Seconds:</b> <input type="text" size="6" maxlength="5" name="length" value="<?=$times[3]?>"><br>
  ...
  $sql = mysql_query("UPDATE ".$table."config SET Config_Value = '".$time_compile."' WHERE Config_ID = 'countdown_time'");
  $sql = mysql_query("UPDATE ".$table."config SET Config_Value = '".$text."' WHERE Config_ID = 'countdown_text'");
  $sql = mysql_query("UPDATE ".$table."config SET COnfig_Value = '".$active."' WHERE Config_ID = 'countdown_active'");
