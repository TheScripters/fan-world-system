...
if (!$_REQUEST['user'] && !$_GET['mode'])
 ...
 elseif ($_REQUEST['user'] && !$_GET['mode'])
  {
    $userinfo = mysql_fetch_array(mysql_query("SELECT UserId,UserName,Name,UserEmail,Active,Act_Key,IP_Login FROM ".$table."users WHERE UserName = '".$_REQUEST['user']."'"));
    ...
    if ($_REQUEST['password'])
      {
        if ($_REQUEST['password'] != $_REQUEST['confpwd'])
          {
            $userinfo = mysql_fetch_array(mysql_query("SELECT UserId,UserName,Name,UserEmail,Active,Act_Key FROM ".$table."users WHERE UserId = '".$_REQUEST['userid']."'"));
...
<input type="submit" value="Submit"></form>
         ...
         else
          {
            ...
            $activedif = mysql_fetch_array(mysql_query("SELECT Active FROM ".$table."users WHERE UserId = '".$_REQUEST['userid']."'"));
            $sql = mysql_query("UPDATE ".$table."users SET UserName = '".$_REQUEST['username']."', UserEmail = '".$_REQUEST['useremail']."', Name = '".$_REQUEST['name']."', Act_Key = '".$_REQUEST['actkey']."', Active = '".$Active."', Password = '".md5($_REQUEST['password'])."' WHERE UserId = '".$_REQUEST['userid']."'");
     ...
     else
      {
        $Active = ($_REQUEST['Active'] == "true") ? "1" : "0";
        $activedif = mysql_fetch_array(mysql_query("SELECT Active FROM ".$table."users WHERE UserId = '".$_REQUEST['userid']."'"));
        $sql = mysql_query("UPDATE ".$table."users SET UserName = '".$_REQUEST['username']."', UserEmail = '".$_REQUEST['useremail']."', Name = '".$_REQUEST['name']."', Act_Key = '".$_REQUEST['actkey']."', Active = '".$Active."' WHERE UserId = '".$_REQUEST['userid']."'");
        ($Active != $activedif['Active']) ? mail_user($_REQUEST['userid'],$Active) : "";
        header("Location: admin.php?page=users");
      }
  }
 elseif ($_GET['mode'] == "delete"){
   ...
   {
	   $user_id = mysql_fetch_array(mysql_query("SELECT UserId FROM ".$table."users WHERE UserName = '".$_REQUEST['UserName']."'"));
	   $user_hosted_files_sql = mysql_query("SELECT AvFilename FROM ".$table."avhosting WHERE UserId = '".$user_id['UserId']."'");
	   while ($user_hosted_files = mysql_fetch_array($user_hosted_files_sql))
	   {
	    	if (file_exists("/home/kpfanwor/public_html/hosting/".$_REQUEST['UserName']."/".$user_hosted_files['AvFilename'].""))
			{
				...
				$sql = mysql_query("DELETE FROM ".$table."avhosting WHERE AvFilename = '".$user_hosted_files['AvFilename']."' and UserId = '".$user_id['UserId']."'");
			}//end if
	   }//end while
	   rmdir("/home/kpfanwor/public_html/hosting/".$_REQUEST['UserName']."");
	}//end if
   $sql = mysql_query("DELETE FROM ".$table."users WHERE UserName = '".$_REQUEST['UserName']."'");
   header("Location: admin.php?page=users");
 }
?>
</center>
