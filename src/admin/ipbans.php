...
if (!$_GET['mode']){
  ...
  ?>  <center>
    ...
    <?php
    $iplstsql = mysql_query("SELECT * FROM ".$table."ip_bans");
...
elseif ($_GET['mode'] == "remove"){
  foreach($_REQUEST['ipaddress'] as $banrem)
    {
      $sql = mysql_query("DELETE FROM ".$table."ip_bans WHERE BanID = '".$banrem."'");
    }
  header("Location: admin.php?page=ipbans");
}
elseif ($_GET['mode'] == "add"){
  $sql = mysql_query("INSERT INTO ".$table."ip_bans VALUES (NULL, '".$_REQUEST['ipaddress']."')");
  header("Location: admin.php?page=ipbans");
}
?>
