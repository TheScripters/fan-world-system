...
if (!$_GET['mode']){
  ...
  ?>  <center>
    ...
    <?php
    $banlist = mysql_query("SELECT UserID FROM ".$table."user_bans");
    while($userlist = mysql_fetch_array($banlist))
      {
        $user = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = '".$userlist['UserID']."'"));
...
elseif ($_GET['mode'] == "remove"){
  foreach($_REQUEST['username'] as $user){
    $userinfo = mysql_fetch_array(mysql_query("SELECT UserName,UserEmail FROM ".$table."users WHERE UserId = '".$user."'"));
    $sql = mysql_query("DELETE FROM ".$table."user_bans WHERE UserID = '".$user."'");
...
elseif ($_GET['mode'] == "add"){
  $username = mysql_fetch_array(mysql_query("SELECT UserId,UserName,UserEmail FROM ".$table."users WHERE UserName = '".$_REQUEST['username']."'"));
  $now = time();
