...
echo "<center>";
$msg_cnt = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS count FROM ".$table."admin_msg"));
if ($msg_cnt['count'] == "0")
  {
 ...
 else
  {
    ...
    $msgs = mysql_query("SELECT * FROM ".$table."admin_msg ORDER BY Msg_ID DESC LIMIT 0,10");
    echo "<table align=\"center\" width=\"50%\"><tr><td>";
    while($msg = mysql_fetch_array($msgs))
      {
        $userinfo = mysql_fetch_array(mysql_query("SELECT UserName,UserEmail FROM ".$table."users WHERE UserId = '".$msg['UserID']."'"));
        $curr_user = mysql_fetch_array(mysql_query("SELECT Time_Zone FROM ".$table."users WHERE UserId = '".$_SESSION['userID']."'"));
...
if ($_GET['mode'] == "add")
  {
    $sql = mysql_query("INSERT INTO ".$table."admin_msg VALUES (NULL, '".$_SESSION['userID']."', '".$_REQUEST['message']."', '".time()."')");
