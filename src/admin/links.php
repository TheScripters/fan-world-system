...
if (!$_GET['mode'] && !$_REQUEST['link']){
  ...
  ?>  <center>
    ...
    $linklist = mysql_query("SELECT LinkID,LinkTitle FROM ".$table."links ORDER BY LinkTitle ASC");
...
elseif (!$_GET['mode'] && $_REQUEST['link']){
  $link = mysql_fetch_array(mysql_query("SELECT LinkTitle,LinkURL,LinkDescription,Banner,Hits,UserId,CatID FROM ".$table."links WHERE LinkID = '".$_REQUEST['link']."'"));
  $cat = mysql_fetch_array(mysql_query("SELECT Link_Cat FROM ".$table."linkcat WHERE CatID = '".$link['CatID']."'"));
  $user = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = '".$link['UserId']."'"));
  ?>
  <h2>Edit Link</h2><br>
  <?php
  include("admin/menu.php");
  ?>  <center>
    ...
    <select name="cat"><?php
    $catlist = mysql_query("SELECT * FROM ".$table."linkcat");
...
elseif ($_GET['mode'] == "remove"){
  foreach ($_REQUEST['link'] as $link){
    $sql = mysql_query("DELETE FROM ".$table."links WHERE LinkID = '".$link."'");
  }
  header("Location: admin.php?page=links");
}
elseif ($_GET['mode'] == "edit"){
  $linkcat = ($_REQUEST['newcat']) ? $_REQUEST['cat'] : $_REQUEST['currcat'];
  $sql = mysql_query("UPDATE ".$table."links SET LinkTitle = '".$_REQUEST['linktitle']."', LinkDescription = '".$_REQUEST['description']."', Banner = '".$_REQUEST['banner']."', Hits = '".$_REQUEST['hits']."', CatID = '".$linkcat."', LinkURL = '".$_REQUEST['linkurl']."' WHERE LinkID = '".$_REQUEST['linkid']."'");
  header("Location: admin.php?page=links");
}
?>
