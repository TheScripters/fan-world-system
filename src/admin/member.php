...
include("admin/menu.php"); 
	$membercount = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS memcnt FROM ".$table."users"));
...
?> 
	...
	<table align="center" rules="rows">
      ...
      if (!$_REQUEST['lstorderby']){
         if(!$_REQUEST['search']){
	     	$memberlist = mysql_query("SELECT * FROM ".$table."users ORDER BY UserId ASC LIMIT $page,30");
	     }else{
	       $memberlist = mysql_query("select * from ".$table."users WHERE ".$_REQUEST['search']." LIKE '%".$_REQUEST['keyword']."%'");
	     }
	     
	   }else if ($_REQUEST['lstorderby']=="namedown"){
	     if(!$_REQUEST['search']){
	     	$memberlist = mysql_query("SELECT * FROM ".$table."users ORDER BY UserName desc LIMIT $page,30");
	     }else{
	       $memberlist = mysql_query("select * from ".$table."users WHERE ".$_REQUEST['search']." LIKE '%".$_REQUEST['keyword']."%'");
	     }
	     ...
	     if(!$_REQUEST['search']){
	     	$memberlist = mysql_query("SELECT * FROM ".$table."users ORDER BY UserName ASC LIMIT $page,30");
	     }else{
	       $memberlist = mysql_query("select * from ".$table."users WHERE ".$_REQUEST['search']." LIKE '%".$_REQUEST['keyword']."%'");
	     }
	     ...
	     if(!$_REQUEST['search']){
	     	$memberlist = mysql_query("SELECT * FROM ".$table."users ORDER BY Joined_Date desc LIMIT $page,30"); 
		 }else{
	       $memberlist = mysql_query("select * from ".$table."users WHERE ".$_REQUEST['search']." LIKE '%".$_REQUEST['keyword']."%'");
	     }
	     ...
	     if(!$_REQUEST['search']){
	     	$memberlist = mysql_query("SELECT * FROM ".$table."users ORDER BY Joined_Date ASC LIMIT $page,30");
		 }else{
	       $memberlist = mysql_query("select * from ".$table."users WHERE ".$_REQUEST['search']." LIKE '%".$_REQUEST['keyword']."%'");
	     }
	     ...
	     if(!$_REQUEST['search']){
	     	$memberlist = mysql_query("SELECT * FROM ".$table."users ORDER BY UserId desc LIMIT $page,30"); 
		 }else{
	       $memberlist = mysql_query("select * from ".$table."users WHERE ".$_REQUEST['search']." LIKE '%".$_REQUEST['keyword']."%'");
	     }
