...
if ($_REQUEST['user'] && !$_GET['mode'])
  {
    $userid = mysql_fetch_array(mysql_query("SELECT UserId,UserName,UserLevel FROM ".$table."users WHERE UserName = '".$_REQUEST['user']."'"));
...
if ($_GET['mode'] == "edit")
  {
    $currlvl = mysql_fetch_array(mysql_query("SELECT UserName,UserEmail,UserLevel FROM ".$table."users WHERE UserId = '".$_REQUEST['userid']."'"));
