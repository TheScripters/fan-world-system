...
if (!isset($_GET['mode'])){
	...
	$users = mysql_query("SELECT DISTINCT UserId FROM ".$table."avhosting");
	$sizes_query = mysql_query("SELECT File_Size FROM ".$table."avhosting");//gets all the data for the file sizes for the user
	$total_size_bytes = 0;
	...
	while($user_id = mysql_fetch_array($users)){
  		$user_name = mysql_fetch_array(mysql_query("select UserName from ".$table."users where UserId = ".$user_id['0'].""));
  		if ($picInRow == 0){
           print "<tr>";
        }//end if
  		print "<td>
	...
	print "</table></td></tr></table>";
  	...
  	print "<h2>Avatar Hosting Managment</h2><br>";
	include("admin/menu.php");
	$avsql = mysql_query("SELECT * FROM ".$table."avhosting WHERE UserId = '".$_REQUEST['userID']."'");//gets all of the pictures for each user
	$user_name = mysql_fetch_array(mysql_query("select UserName from ".$table."users where UserId = '".$_REQUEST['userID']."'"));
	$sizes_query = mysql_query("SELECT File_Size FROM ".$table."avhosting WHERE UserId = '".$_REQUEST['userID']."'");//gets all the data for the file sizes for the user
	$total_size_bytes = 0;
	//adds up how much space it used by the user's pictures, and adds up how many pictures they have uploaded
	while ($pic_sizes = mysql_fetch_array($sizes_query)){
 		$total_size_bytes = $total_size_bytes + $pic_sizes['0'];
	...
	print "<center>Confim Delete and User E-mail Message
  	...
  	$uid = mysql_fetch_array(mysql_query("select UserId from ".$table."users where UserName = '".$_REQUEST['user']."'"));
  	$userinfo = mysql_fetch_array(mysql_query("SELECT UserName,UserEmail FROM ".$table."users WHERE UserId = '".$uid['0']."'"));
    ...
    if (file_exists("/home/kpfanwor/public_html/hosting/".$_REQUEST['user']."/".$_REQUEST['pic']."")){//makes sure the file has not been deleted already, or is missing
		$query = "DELETE from ".$table."avhosting where AvFilename = '".$_REQUEST['pic']."' and UserId = ".$uid['0']."";//deletes entry from database
	...
	$query = "DELETE from ".$table."avhosting where AvFilename = '".$_REQUEST['pic']."' and UserId = ".$uid['0']."";
	header("Location: admin.php?page=avhost"); 
	}//end if
}//end if
?>