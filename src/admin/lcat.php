...
if (!$_GET['mode'] && !$_REQUEST['cat']){
  ...
  ?>    <center>
    ...
    $catlist1 = mysql_query("SELECT * FROM ".$table."linkcat");
    ...
    $catlist2 = mysql_query("SELECT * FROM ".$table."linkcat");
...
elseif (!$_GET['mode'] && $_REQUEST['cat']){
  $cat = mysql_fetch_array(mysql_query("SELECT * FROM ".$table."linkcat WHERE CatID = '".$_REQUEST['cat']."'"));
  ?>
...
elseif ($_GET['mode'] == "remove"){
  foreach ($_REQUEST['cat'] as $cat)
    {
      $sql = mysql_query("DELETE FROM ".$table."linkcat WHERE CatID = '".$cat."'");
      $sql = mysql_query("UPDATE ".$table."links SET CatID = '".$_REQUEST['newcat']."' WHERE CatID = '".$cat."'");
    }
  header("Location: admin.php?page=lcat");
}
elseif ($_GET['mode'] == "edit"){
  $sql = mysql_query("UPDATE ".$table."linkcat SET Link_Cat = '".$_REQUEST['cat_title']."' WHERE CatID = '".$_REQUEST['catid']."'");
  header("Location: admin.php?page=lcat");
}
elseif ($_GET['mode'] == "add"){
  $sql = mysql_query("INSERT INTO ".$table."linkcat VALUES (NULL, '".$_REQUEST['newcat']."')");
  header("Location: admin.php?page=lcat");
}
?>
