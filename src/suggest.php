    ...
    if (valid_email($email))
      {
        if ($visual == $_SESSION['visual_confirmation'])
          {
            mail("staff@kpfanworld.com","Feedback from ".$_SESSION['siteTitle']."",strip_gpc_slashes("Dear Staff,\n\n$name ($email) has provided feedback using the ".$_SESSION['siteTitle']." feedback page. Their message is as follows:\n\n$feedback\n\n--------\nThank you.\n\nIP: ".$_SERVER['REMOTE_ADDR'].""),$_SESSION['headers']);
            mail($email,"Feedback Received: ".$_SESSION['siteTitle']."",strip_gpc_slashes("Dear $name,\n\nThe following message has been receieved by the staff of ".$_SESSION['siteTitle'].":\n\n$feedback\n-------\nThank you.\n\nIP: ".$_SERVER['REMOTE_ADDR'].""),$_SESSION['headers']);
