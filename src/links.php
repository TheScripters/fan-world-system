...
echo "<table width=\"50%\">";
$linkCat = mysql_query("SELECT * FROM ".$table."linkcat ORDER BY Link_Cat ASC");
while ($LinkCat = mysql_fetch_array($linkCat))
  {
    $Links = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS Links FROM ".$table."links WHERE CatID = '".$LinkCat['CatID']."'"));
    ...
    $Link = mysql_query("SELECT * FROM ".$table."links WHERE CatID = '".$LinkCat['CatID']."' ORDER BY LinkTitle ASC");
