...
if (!isset($_GET['mode']))
  {
    ...
    if ($_GET['ep'] == 1){
      	$ep_count = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS count FROM ".$table."eplist"));//gets the total number of episodes
    ...
    if (isset($_GET['ep']) && $_GET['ep'] != "movie" && !isset($_GET['id']))
      {
        ...
        $epinfo1 = mysql_fetch_array(mysql_query("SELECT * FROM ".$table."eplist WHERE EpId = '".$_GET['ep']."'"));
        $epinfo2 = mysql_fetch_array(mysql_query("SELECT * FROM ".$table."epguide WHERE EpId = '".$_GET['ep']."'"));
        ...
        $stars = mysql_fetch_array(mysql_query("SELECT Stars FROM ".$table."epguide WHERE EpId = '".$_GET['ep']."'"));
        ...
        $guest = mysql_fetch_array(mysql_query("SELECT GuestStars FROM ".$table."epguide WHERE EpId = '".$_GET['ep']."'"));
        ...
        $notesql = mysql_query("SELECT NoteId,NoteText,UserId FROM ".$table."epnotes WHERE EpId = '".$_GET['ep']."'");
        // Note loop
        while($note = mysql_fetch_array($notesql))
          {
            $usern = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = '".$note['UserId']."'"));
        ...
        $quotesql = mysql_query("SELECT QuoteID,QuoteText,UserId FROM ".$table."quotes WHERE EpId = '".$_GET['ep']."'");
        // Quote loop
        while($quote = mysql_fetch_array($quotesql))
          {
            $userq = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = '".$quote['UserId']."'"));
            // Change newlines to HTML line break, parse BBCode
        ...
        $goofsql = mysql_query("SELECT GoofID,GoofText,UserId FROM ".$table."goofs WHERE EpId = '".$_GET['ep']."'");
        // Goof loop
        while($goof = mysql_fetch_array($goofsql))
          {
            $userg = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = '".$goof['UserId']."'"));
            // Change newlines to HTML line break, parse BBCode
        ...
        $cultsql = mysql_query("SELECT CultId,CulturalText,UserId FROM ".$table."cultural WHERE EpId = '".$_GET['ep']."'");
        // Cultural Reference loop
        while($cultural = mysql_fetch_array($cultsql))
          {
            $userc = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = '".$cultural['UserId']."'"));
            // Change newlines to HTML line break, parse BBCode
     ...
     elseif ($_GET['ep'] == "movie" && isset($_GET['id']))
      {
        ...
        $movie = mysql_fetch_array(mysql_query("SELECT * FROM ".$table."dvdmovie WHERE Id = '".$_GET['id']."'"));
        $movie1 = mysql_fetch_array(mysql_query("SELECT * FROM ".$table."dvdguide WHERE DVD_ID = '".$_GET['id']."'"));
        ...
        if ($movie1['Type'] == "Individual")
          {
            ...
            for ($i=0; $i<$size; $i++)
             {
               $ep = mysql_fetch_array(mysql_query("SELECT EpId FROM ".$table."eplist WHERE EpTitle = '".$titles[$i]."'"));
        ...
        echo "<tr><td align=\"center\"><table align=\"center\" width=\"50%\">";
        // Request Notes from database
        $dvdnotes = mysql_query("SELECT DVDNoteID,NoteText,UserId FROM ".$table."dvdnotes WHERE DVD_ID = '".$_GET['id']."'");
        // Notes loop
 ...
 elseif ($_GET['mode'] == "list")
  {
    ...
    $eplist1 = mysql_query("SELECT * FROM ".$table."eplist WHERE Season = 'Season 1' ORDER BY EpId ASC");
    ...
    $eplist2 = mysql_query("SELECT * FROM ".$table."eplist WHERE Season = 'Season 2' ORDER BY EpId ASC");
    ...
    $eplist3 = mysql_query("SELECT * FROM ".$table."eplist WHERE Season = 'Season 3' ORDER BY EpId ASC");
    ...
    $eplist4 = mysql_query("SELECT * FROM ".$table."eplist WHERE Season = 'Season 4' ORDER BY EpId ASC");
    ...
    $eplistdvd = mysql_query("SELECT * FROM ".$table."dvdmovie ORDER BY Id ASC");
