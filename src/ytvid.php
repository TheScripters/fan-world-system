...
if ($_SESSION['logged_in'] && !isset($_GET['vid']) && !isset($_REQUEST['mode']))
  {
    ...
    ?><br>This form allows you to "upload" a video to ".$_SESSION['siteTitle']." that's already online at YouTube.com. Find the video ID from the URL and enter it below.<br><br>You can find it by looking after the "watch?v=" For example, you would enter the <b>bold</b> string:<br>http://www.youtube.com/watch?v=<b>wPCD9LKlg5A</b><br><br>Hit submit, and that's it. We'll do the rest.<br>
...
if ($_SESSION['logged_in'] && $_REQUEST['mode'] == "submit")
  {
    $vid = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS count FROM ".$table."youtubevid WHERE YT_Vid_ID = '".$_REQUEST['vid']."'"));
    if ($vid['count'] == "0")
      {
        $sql = mysql_query("INSERT INTO ".$table."youtubevid VALUES (NULL, '".$_SESSION['userID']."', '".$_REQUEST['vid']."', '".$_REQUEST['title']."', '".$_REQUEST['description']."')");
