...
if (empty($_REQUEST['Link_Title']) || empty($_REQUEST['Link_URL']) && isset($_SESSION['logged_in']))
  {
    ...
    $CatSel = mysql_query("SELECT * FROM ".$table."linkcat ORDER BY CatID ASC");
...
if (!empty($_REQUEST['Link_Title']) && !empty($_REQUEST['Link_URL']) && isset($_SESSION['logged_in']))
  {
    if ($_REQUEST['Link_Category'] == "New")
      {
        $AddCat = mysql_query("INSERT INTO ".$table."linkcat VALUES (NULL, '".$_REQUEST['new_select']."')");
        $CatID = mysql_fetch_array(mysql_query("SELECT CatID FROM ".$table."linkcat WHERE Link_Cat = '".$_REQUEST['new_select']."'"));
    ...
    $LinkAdd = mysql_query("INSERT INTO ".$table."links VALUES (Null, '$Category', '".$_REQUEST['Link_Title']."', '".$_REQUEST['Link_URL']."', '".$_REQUEST['Link_Description']."', '".$_REQUEST['Link_Banner']."', '0', '".$_SESSION['userID']."')");
    incheader("Add Link");
