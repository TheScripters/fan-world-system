...
if ($_GET['mode'] == "userbars")
  {
    $count = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS count FROM ".$table."userbars"));
    ...
    $user = mysql_query("SELECT * FROM ".$table."userbars ORDER BY BarName ASC LIMIT 0,".$barcount."");
    ...
    $user1 = mysql_query("SELECT * FROM ".$table."userbars ORDER BY BarName ASC LIMIT ".$barcount.",".$barcnt."");
 ...
 elseif ($_GET['mode'] == "avatars")
  {
    $count = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS count FROM ".$table."avatars"));
    incheader("Avatars");
    ...
    $picInRow = 1;
    $avsql = mysql_query("SELECT * FROM ".$table."avatars ORDER BY Avatar_Name ASC");
    while($avatar = mysql_fetch_array($avsql))
    //for ($i=1;$i<$count['count'];$i++)
     {
       //$avatar = mysql_fetch_array(mysql_query("SELECT Avatar_Name,Avatar_FileName FROM ".$table."avatars WHERE Avatar_ID = '".$i."'"));
       if ($picInRow == 1)
         {
  	       echo "<tr>";
         }
