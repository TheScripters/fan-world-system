...
if ($_GET['mode'] == "delete"){
	if (file_exists("/home/kpfanwor/public_html/hosting/".$_SESSION['UserName']."/".$_GET['pic']."")){//makes sure the file has not been deleted already, or is missing
		$query = "DELETE from ".$table."avhosting where AvFilename = '".$_GET['pic']."' and UserId = ".$_SESSION['userID']."";//deletes entry from database
	...
	$query = "DELETE from ".$table."avhosting where AvFilename = '".$_GET['pic']."' and UserId = ".$_SESSION['userID']."";
	header("Location: avhosting"); 
	}//end if
