...
if ($_SESSION['logged_in']){
	$user_time_zone = mysql_fetch_array(mysql_query("SELECT Time_Zone FROM ".$table."users WHERE UserId = ".$_SESSION['userID'].""));
}else{
  $user_time_zone = -0.00;
}//end if
$timenow = time();
...
if (!$_GET['mode']){
  $ficTitle = mysql_fetch_array(mysql_query("SELECT Title FROM ".$table."fftitles WHERE FF_Id = '".$_GET['ficID']."'"));
$story_title_userID = mysql_fetch_array(mysql_query("SELECT ChapterTitle, UserID FROM ".$table."ffchapter where FicId = ".$_GET['ficID']." and Chapter = ".$_GET['chap'].""));
$author_info = mysql_fetch_array(mysql_query("SELECT UserName, UserEmail, ShowEmail FROM ".$table."users WHERE UserId = ".$story_title_userID['UserID'].""));
