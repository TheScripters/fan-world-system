...
if ($_GET['mode'] == "login")
  {
    ...
    $sqlpwd = mysql_fetch_array(mysql_query("SELECT UserId, Password, Active, UserLevel, Time_Zone FROM ".$table."users WHERE UserName = '$Uname'"));
    if ($Pass == $sqlpwd['Password'] && $sqlpwd['Active'] == "1")
      {
        ...
        $IP_Login = mysql_query("UPDATE ".$table."users SET IP_Login = '".$_SERVER['REMOTE_ADDR']."' WHERE UserId = '".$sqlpwd['UserId']."'");
