<?php
// Code © 2007 ".$_SESSION['siteTitle']."
// Code written by Adam Humpherys
/*******fic.php******************/
include "includes/functions.php";
if ($_GET['fic'] != "list" && $_GET['fic'] && $_GET['chap']){
  incheader("View Story");
  $ficinfo = mysql_fetch_array(mysql_query("SELECT Title,UserId FROM ".$table."fftitles WHERE FF_Id = '".$_GET['fic']."'"));
  $chapinfo = mysql_fetch_array(mysql_query("SELECT Date,ChapterTitle,ChapterText FROM ".$table."ffchapter WHERE FicId = '".$_GET['fic']."' AND Chapter = '".$_GET['chap']."'"));
  $user = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = '".$ficinfo['UserId']."'"));
  ...
  echo "</td>";
  $totchap = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS count FROM ".$table."ffchapter WHERE FicId = '".$_GET['fic']."'"));
  if ($totchap['count'] >= 2) {
    echo "<td align=\"center\" width=\"34%\"><form name=\"jump1\"><select name=\"menu1\">";
    $chapsql = mysql_query("SELECT Chapter,ChapterTitle FROM ".$table."ffchapter WHERE FicId = '".$_GET['fic']."' AND Chapter != '".$_GET['chap']."' ORDER BY Chapter ASC");
  ...
  $chap = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS count FROM ".$table."ffchapter WHERE FicId = '".$_GET['fic']."' AND Chapter = '".$nextchap."'"));
  if ($chap['count'] == 1){
  ...
  $listsql = mysql_query("SELECT FF_Id,Title,UserId FROM ".$table."fftitles WHERE Public = 'Yes' ORDER BY Title ASC");
  echo "<ul>";
  while($list = mysql_fetch_array($listsql))
    {
      $chapsql = mysql_query("SELECT ChapterTitle,Chapter FROM ".$table."ffchapter WHERE FicId = '".$list['FF_Id']."'");
