...
if (!$_SESSION['logged_in']){
  ...
  $sql = mysql_query("INSERT INTO ".$table."fftitles VALUES (NULL, '".$fic['FicTitle']."', '".$cnt['cnt']."', '".$_SESSION['userID']."')");
  ...
  while($chapter = mysql_fetch_array($chapsql))
    {
      $sql = mysql_query("INSERT INTO ".$table."ffchapter VALUES ('".$ficId."', '".$chapter['ChapNum']."', '".$_SESSION['userID']."', '".time()."', '".$chapter['chapter']."')");
  ...
  foreach ($_REQUEST['fic'] as $fic)
    {
      ...
      $ficins = mysql_query("INSERT INTO ".$table."fftitles VALUES(NULL, '".$sql['ficTitle']."','0','".$_SESSION['userID']."','Yes')");
      $ficid = mysql_insert_id();
      ...
      while($fics = mysql_fetch_array($ficsql))
        {
          ...
          $chap = mysql_fetch_array(mysql_query("SELECT Chapter FROM ".$table."ffchapter WHERE FicId = '".$ficid."' ORDER BY Chapter DESC LIMIT 1"));
          $chapnum = $chap['Chapters'] + 1;
          $chins = mysql_query("INSERT INTO ".$table."ffchapter VALUES ('".$ficid."', '".$chapnum."','".$_SESSION['userID']."', '".$time."', '".$fics['chapter']."', '".addslashes($fictext)."')");
          echo "<li>".$fics['chapter']."</li>";
          $ficchap = mysql_fetch_array(mysql_query("SELECT Chapters FROM ".$table."fftitles WHERE FF_Id = '".$ficid."'"));
          $numch = $ficchap['Chapters'] + 1;
          $sql = mysql_query("UPDATE ".$table."fftitles SET Chapters = '".$numch."' WHERE FF_Id = '".$ficid."'");
        }
      echo "</ol>";
    }
  echo "</ol><br><br>\n";
  ...
  echo "<a href=\"http://fanfic.kpfanworld.com\">Click here</a> to return to ".$_SESSION['siteTitle']."'s FanFiction site.";
  exit;
}
echo "</center>";
?>
