...
if (!$_SESSION['logged_in']){
  ...
  if ($_REQUEST['fic'] == "new"){
    $sql = mysql_query("INSERT INTO ".$table."fftitles (NULL, '".$_REQUEST['fictitle']."', '0' '".$_SESSION['userID']."','Yes')");
    $ficid = mysql_insert_id();
    $chapter = 1;
  } else {
    $ficid = $_REQUEST['fic'];
    $chapsel = mysql_fetch_array(mysql_query("SELECT Chapter FROM ".$table."ffchapter WHERE FicId = '".$ficid."' ORDER BY Chapter LIMIT 1"));
    $chapter = $chapsel['Chapter'] + 1;
  }
  if ($_REQUEST['title_only'] == "true") {
    $sql = mysql_query("UPDATE ".$table."fftitles SET Public = 'No' WHERE FF_Id = '".$ficid."'");
    header("Location: index.php");
  }
  $sql = mysql_query("INSERT INTO ".$table."ffchapter VALUES ('".$ficid."', '".$chapter."', '".$_SESSION['userID']."', '".$_REQUEST['chaptitle']."', '".addslashes($_REQUEST['chaptext'])."')");
  $chapcnt = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS cnt FROM ".$table."ffchapter WHERE FicId = '".$ficid."'"));
  $cnt = $chapcnt['cnt'] + 1;
  $sql = mysql_query("UPDATE ".$table."fftitles SET Chapters = '".$cnt."' WHERE FF_Id = '".$ficid."'");
  ...
  $ficsql = mysql_query("SELECT FF_Id,Title,Chapters FROM ".$table."fftitles WHERE UserId = '".$_SESSION['userID']."'");
