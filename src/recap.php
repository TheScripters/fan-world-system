...
if ($_GET['mode'] == "recap" && isset($_GET['ep']))
  {
    ...
    $epinfo = mysql_fetch_array(mysql_query("SELECT EpTitle FROM ".$table."eplist WHERE EpId = '".$_GET['ep']."'"));
    $recap = mysql_fetch_array(mysql_query("SELECT EpRecap FROM ".$table."epguide WHERE EpId = '".$_GET['ep']."'"));
 ...
 elseif ($_GET['mode'] == "script" && isset($_GET['ep']))
  {
    ...
    $epinfo = mysql_fetch_array(mysql_query("SELECT EpTitle FROM ".$table."eplist WHERE EpId = '".$_GET['ep']."'"));
    $script = mysql_fetch_array(mysql_query("SELECT ScriptText FROM ".$table."transcript WHERE EpId = '".$_GET['ep']."'"));
 ...
 elseif ($_GET['mode'] == "scriptlist")
  {
    $count = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS count FROM ".$table."transcript"));
    $scrptcount = round($count['count'] / 2);
    $scrptcnt = $count['count'] - $scrptcount;
    incheader("Transcript List");
    include("includes/guides_table.inc");
    $scriptcount = mysql_fetch_array(mysql_query("SELECT COUNT(ScriptText) AS count FROM ".$table."transcript WHERE ScriptText != ''"));
    echo "<br><b>".$scriptcount['count']."</b> Transcripts Online<br><hr width=\"200\"><br>";
    $scriptsql = mysql_query("SELECT * FROM ".$table."transcript ORDER BY EpId ASC LIMIT 0,".$scrptcount);
    echo "<table align=\"center\"><tr><td><ul style=\"list-style:none\">";
    while($script = mysql_fetch_array($scriptsql))
      {
        $epinfo = mysql_fetch_array(mysql_query("SELECT EpTitle,EpNumber FROM ".$table."eplist WHERE EpId = '".$script['EpId']."'"));
    ...
    $scriptsql1 = mysql_query("SELECT * FROM ".$table."transcript ORDER BY EpId ASC LIMIT ".$scrptcount.",".$scrptcnt);
    while($script1 = mysql_fetch_array($scriptsql1))
      {
        $epinfo1 = mysql_fetch_array(mysql_query("SELECT EpTitle,EpNumber FROM ".$table."eplist WHERE EpId = '".$script1['EpId']."'"));
