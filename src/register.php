        ...
        if ($Pass == $Pass1 && !empty($UserName) && !empty($UEmail) && !empty($UEmail2) && $UEmail == $UEmail2 && $Code_Confirm == $_SESSION['visual_confirmation'])
          {
            ...
            $sql_user = mysql_query("SELECT COUNT(*) AS count FROM ".$table."users WHERE UserName = '".$UserName."'");
            $User = mysql_fetch_array($sql_user);
            $sql_email = mysql_query("SELECT COUNT(*) AS count FROM ".$table."users WHERE UserEmail = '".$UEmail."'");
            ...
            if ($User['count'] == "1" || $UserEmail['count'] == "1")
             ...
             else
              {
                ...
                $sql_reg = mysql_query("INSERT INTO ".$table."users VALUES (NULL , '".$UserName."', '".$UEmail."', MD5( '".$Pass."' ) , '$joined', '', '', '".$Show_Email."', '$Name', '', '$Act_Key', '0', '1', '0')");
                $userID = mysql_fetch_array(mysql_query("SELECT UserID FROM ".$table."users WHERE UserEmail = '$UEmail'"));
