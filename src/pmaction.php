...
if (isset($_SESSION['logged_in']) && isset($_GET['mode']))
  {
    if ($_GET['mode'] == "delete" && isset($_GET['id']))
      {
        $PMInfo = mysql_fetch_array(mysql_query("SELECT Subject FROM ".$table."privmsg WHERE MsgID = '".$_GET['id']."'"));
    ...
    if ($_GET['mode'] == "delcon" && isset($_GET['id']))
      {
        $sql = mysql_query("DELETE FROM ".$table."privmsg WHERE MsgID = '".$_GET['id']."'");
        $sql = mysql_query("DELETE FROM ".$table."pmsgtext WHERE MsgID = '".$_GET['id']."'");
        incheader("Delete Message");
    ...
    if ($_GET['mode'] == "reply" && isset($_GET['id']))
      {
        $PMInfo = mysql_fetch_array(mysql_query("SELECT * FROM ".$table."privmsg WHERE MsgId = '".$_GET['id']."'"));
        $Receipt = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = '".$PMInfo['From_User']."'"));
        $MsgText = mysql_fetch_array(mysql_query("SELECT Msg_Text FROM ".$table."pmsgtext WHERE MsgID = '".$_GET['id']."'"));
    ...
    if ($_GET['mode'] == "send")
      {
        $now = time();
        $Receipt = mysql_fetch_array(mysql_query("SELECT UserId,UserEmail FROM ".$table."users WHERE UserName = '".$_REQUEST['rptName']."'"));
        $sql = mysql_query("INSERT INTO ".$table."privmsg VALUES (NULL, '".$_SESSION['userID']."', '".$Receipt['UserId']."', '".$_REQUEST['Subject']."', '".$now."', '1', '0')");
        $MsgID = mysql_insert_id();
        $sql1 = mysql_query("INSERT INTO ".$table."pmsgtext VALUES ('".$MsgID."', '".$_REQUEST['MsgText']."')");
        mail("".$_GET['rptName']." <".$Receipt['UserEmail'].">","New private message on ".$_SESSION['siteTitle']." From ".$_SESSION['UserName']."",strip_gpc_slashes("Hello ".$_REQUEST['rptName'].",\n\n".$_SESSION['UserName']." has sent you a private message on ".$_SESSION['siteTitle'].".\n\nSubject: ".$_REQUEST['Subject']."\n\nGo here and login to view: http://www.kpfanworld.com/privmsg.php?id=".$MsgID."\n\nThank you\nManagement\n".$_SESSION['siteTitle'].""), "From: webmaster@kpfanworld.com");
        if ($_GET['send'] == "reply")
          {
            $reply = mysql_query("UPDATE ".$table."privmsg SET Replied = '1' WHERE MsgID = '".$_REQUEST['origMsgID']."'");
          }
        incheader("Compose Message");
