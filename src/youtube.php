...
if (!$_GET['vid'])
  {
    ...
    $vidsql = mysql_query("SELECT * FROM ".$table."youtubevid ORDER BY Vid_ID ASC");
    while($vid = mysql_fetch_array($vidsql))
      {
        $user = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = '".$vid['UserId']."'"));
        echo $vid['Vid_ID'].". <a href=\"youtube/".$vid['Vid_ID']."#vid\">".$vid['Video_Title']."</a> -- Uploaded by <a href=\"profile/".$vid['UserId']."\">".$user['UserName']."</a><br>";
      }
    echo "<br><br><a href=\"ytvid.php\">Submit video</a></td></tr></table>";
  }
if ($_GET['vid'])
  {
    $vid = mysql_fetch_array(mysql_query("SELECT * FROM ".$table."youtubevid WHERE Vid_ID = '".$_GET['vid']."'"));
