...
if (!$_GET['mode'] && $_GET['type'] == "audio"){
  ...
  $audiosql = mysql_query("SELECT * FROM ".$table."audio");
  while ($audio = mysql_fetch_array($audiosql))
    {
      $user = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = '".$audio['UserId']."'"));
  ...
  echo "<br><br><h3>File List</h3>";
  echo "<table align=\"center\" width=\"50%\"><tr><td><ul style=\"list-style:none\">";
  $videosql = mysql_query("SELECT * FROM ".$table."video");
  while ($video = mysql_fetch_array($videosql))
    {
      $user = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = '".$video['UserId']."'"));
  ...
  $dialogsql = mysql_query("SELECT * FROM ".$table."dialog");
  while ($dialog = mysql_fetch_array($dialogsql))
    {
      $user = mysql_fetch_array(mysql_query("SELECT UserName FROM ".$table."users WHERE UserId = '".$dialog['UserId']."'"));
  ...
  $sql = mysql_query("INSERT INTO ".$table."".$tbl." VALUES (NULL, '".basename($_FILES['media']['name'])."', '".$_REQUEST['filetitle']."', '".$_REQUEST['filedesc']."', '".$_SESSION['userID']."')");
