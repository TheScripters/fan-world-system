...
$newsactive = mysql_fetch_array(mysql_query("SELECT Config_Value as active FROM ".$table."config WHERE Config_ID = 'Index_News_Active'"));
...
if ($newsactive['active'] == "True")
  {
    $news = mysql_fetch_array(mysql_query("SELECT Config_Value as news FROM ".$table."config WHERE Config_ID = 'Index_News'"));
    echo "<br><font size=\"+1\" color=\"#00FF00\">".stripslashes(nl2br($news['news']))."</font><br>\n";
  }
$today = gmdate("M d", time()+$_SESSION['TZone']);
$hist_count = mysql_query("SELECT COUNT(*) AS count FROM ".$table."today WHERE Month_Day = '$today'");
...
if ($histCount['count'] >= "1")
  {
    $history = mysql_query("SELECT Month_Day,Year,News_Text FROM ".$table."today WHERE Month_Day = '$today'");
